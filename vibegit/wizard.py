...
class ConfigWizard:
    ...
    def _configure_api_keys(self):
        ...
        if not api_key_env:
            console.print("[yellow]No API key configuration needed for this model.[/yellow]")
            return
            
        # Check if the API key is already in the environment
        if api_key_env in os.environ and os.environ[api_key_env]:
            console.print(f"[green]Found {api_key_env} in environment variables.[/green]")
            ...
            if answers and answers["save_api_key"]:
                ...
                console.print(f"[yellow]{api_key_env} will be used from environment variables.[/yellow]")
            
            return
            
        # API key not found in environment, prompt for it
        console.print(f"[yellow]No {api_key_env} found in environment variables.[/yellow]")
        ...
        if api_key:
            ...
            console.print(f"[red]No {api_key_env} provided. You'll need to set it later.[/red]")
