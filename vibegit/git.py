...
class ChangedGitFile(BaseGitFile):
    def get_diff(self) -> FileDiff:
        output = self.repo.git.diff(
            "HEAD", "--no-color", "--binary", "--", self.filename
        )
...
def get_git_status(repo) -> GitStatusSummary:
    ...
    return GitStatusSummary(
        repo=repo,
        changed_files=[ChangedGitFile(repo, path) for path in changed_paths],
...
class CommitProposalContext:
    ...
    def validate_commit_proposal(self, commit_proposals: CommitProposalsResultSchema):
        groups = [proposal.change_ids for proposal in commit_proposals.commit_proposals]
        if isinstance(commit_proposals, IncompleteCommitProposalsResultSchema):
            groups.extend(
                group.change_ids for group in commit_proposals.excluded_groups
            )

        seen_change_ids: set[int] = set()
        for group in groups:
            for change_id in group:
                ...
                if change_id in seen_change_ids:
                    raise ValueError(
                        f"Change ID {change_id} is included in multiple proposal or exclusion groups"
                    )
                seen_change_ids.add(change_id)

        missing_change_ids = self.change_id_to_ref.keys() - seen_change_ids
        if missing_change_ids:
            formatted_ids = ", ".join(map(str, sorted(missing_change_ids)))
            raise ValueError(
                f"Changes not covered by the model response: {formatted_ids}"
            )
